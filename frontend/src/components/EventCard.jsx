...
const EventCard = ({ event }) => {
	return (
		<div className='border border-slate-200 rounded-3xl bg-white p-3 space-y-3 hover:shadow-[0_0px_14px_rgba(0,0,0,0.25)]  transition-shadow duration-300 ease-in-out'>
			...
			<Link to={`/events/${event.id}`} className='text-xl font-semibold'>
				{event?.title}
			</Link>
			<div>
			...
			<div className='rounded-full text-center border-2 border-purple-700 text-white bg-purple-700 hover:bg-purple-700/90  transition-colors duration-300 ease-in-out px-3 py-1.5 cursor-pointer font-medium'>
				{event.price} Dh
			</div>
			{/* </div> */}
		</div>
